...
/**
 * REST controller for managing {@link com.clarabernerd.myapp.domain.Order}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class OrderResource {
    ...
    public ResponseEntity<Order> partialUpdateOrder(@PathVariable(value = "id", required = false) final Long id, @RequestBody Order order)
        ...
        Optional<Order> result = orderRepository
            ...
            .map(existingOrder -> {
                if (order.getPickupAddr() != null) {
                    existingOrder.setPickupAddr(order.getPickupAddr());
                }
                if (order.getDeliveryAddr() != null) {
                    existingOrder.setDeliveryAddr(order.getDeliveryAddr());
                }
